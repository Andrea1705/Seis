...
select * from clientes;
 ...
 (codigo, 'Carla', 'Conti', '33456456', 'Amapola 456'),
 (codigo, 'Fernando', 'Roldán', '46777834', 'Verdaguer 4467'),
 (codigo, 'Sandra', 'Benitez', '32456372', 'Zapiola 357'),
 (codigo, 'Luna', 'Falbán', '78954332', 'El cerro 3352'),
 (codigo, 'Leandro', 'Francesco', '61243223', 'Hernández 5967');
...
-- es decir de la forma extendida. Completando todos los campos
 ...
 insert into clientes (codigo, nombre, apellido, dni, direccion) values
 (codigo, 'Fabiana', 'Soldán', '98765484', 'Andonaegui 45');
 insert into clientes (codigo, nombre, apellido, dni, direccion) values
 (codigo, 'Pedro', 'Herrero', '56756834', 'Cuenca 345');
 insert into clientes (codigo, nombre, apellido, dni, direccion) values
 (codigo, 'Guido', 'Albán', '45637832', 'Bolivia 3561');
 insert into clientes (codigo, nombre, apellido, dni, direccion) values
 (codigo, 'Axel', 'Kandiski', '35624841', 'Tres Arroyos 2467');
 insert into clientes (codigo, nombre, apellido, dni, direccion) values
 (codigo, 'Anabel', 'Kundera', '73254333', 'Bahía Blanca 258');
...
update clientes set nombre = 'José' where codigo = 1;
Select * from clientes where codigo = 1;
...
Select * from clientes where codigo = 3 or codigo = 5 or codigo = 7;
-- no tengo código = 5 ¿? it is null
Select * from clientes where codigo = 5;
update clientes set direccion = 'is null'  where codigo in (3, 7);
Select * from clientes where codigo = 3 or codigo = 5 or codigo = 7;

-- 7-	Eliminar los clientes con apellido Perez.

Select * from clientes where apellido = 'Perez';
-- no hay clientes con apellido Perez

-- 8-	Eliminar los clientes con dni que terminan en 0.

Select * from clientes where dni like '%0';
-- no tengo registros con dni terminados en 0

-- 9-  Eliminar los clientes cuyo nombre contenga una r y su apellido termine en z

Select * from clientes where nombre like '%r%' and apellido like '%z';
-- tengo 6 registros que cumplen la condición 4, 8, 10, 13, 14, 18.
delete from clientes where nombre like '%r%' and apellido like '%z';
-- desactivar actualizacion segura
Select * from clientes where nombre like '%r%' and apellido like '%z';
...
-- No tengo artículos con precio menor o igual a 50, igual me muestra todos los articulos 
-- Select * from articulos (precio) limit 0,50; VER

-- 	11- Aumentar un 15% los precios de los artículos con precio mayor a 50.

Select * from articulos where precio > 50;
-- modo de actualización segura? 
-- set sql_safe_updates=0; NO
Select * from articulos where precio > 50;
update articulos set precio = precio + precio *0.15 where precio > 50;
Select * from articulos where precio > 50;
-- alfajorcito 1000 = 1150
...
Select * from articulos where precio > 200;
-- alfajorcito 1092,5 pesos.

-- 13- bajar el %50 del precio a los artículos que estén entre 20000 y 40000

Select * from articulos where precio between 20000 and 40000;
-- set sql_safe_updates=0; 
update articulos set precio = precio - (precio *0.5) where precio between 20000 and 40000;
Select * from articulos where precio between 20000 and 40000;
-- yerba 27312,5 - 50% 13656.25

-- 14- aumentar el stock a 200 a todos aquellos productos que tengan el stock mayor a 0 y hasta 150

Select * from articulos where stock > 0 and stock < 150;
-- Tengo 5 artículos con stock mayor a 0 y menor a 150. Productos alfajorcitos, papitas, agua,
-- dulce de leche, yerba. OJO LLEVAR EL STOCK A 200

Update articulos set stock = 200 where stock > 0 and stock < 150;
Select * from articulos where stock = 200;

-- 	14- Eliminar los artículos con stock menor a 0.

Select * from articulos where stock < 0;
-- No tengo artículos con stock menor a 0

-- 15- eliminar todos los artículos cuyo precio sea mayor a $50.000

Select * from articulos where precio > '50000';
delete from articulos where precio > '50000';
Select * from articulos where precio > '70000';
